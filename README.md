...
Compose starts with fixed data paths, ports, the Mock OA URL and demo shared secret. It does not require a `.env` file to start. Keep only the administrator username/password in environment variables for the first secure login; the JWT signing key is generated and persisted automatically.
...
首次启动时，WorkHub 使用 `WORKHUB_ADMIN_USERNAME` 和 `WORKHUB_ADMIN_PASSWORD` 原子创建唯一初始管理员；数据库已有管理员后不会再次引导。管理 API 使用 HttpOnly JWT Cookie 和严格同源 Origin 校验。通过 HTTPS 部署时必须设置 `WORKHUB_ADMIN_COOKIE_SECURE=true`。
