...
运行配置必须区分固定启动参数与动态配置。Chat、Embedding、飞书凭据及运行时超时、重连、迭代参数必须持久化到 `app.db`，通过管理员 Web API 修改，使用 revision 和审计，且密钥只返回配置状态。保存后配置必须作用于后续 run，已开始的 run 不得被强制终止。未配置模型或飞书时服务仍必须能够启动并提供管理端。
