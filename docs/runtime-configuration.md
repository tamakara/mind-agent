...
Compose supplies `WORKHUB_DATA_DIR`, `WORKHUB_STATIC_DIR`, `WORKHUB_HOST`, `WORKHUB_PORT`, the Mock OA URL and demo shared secret. The only expected environment values are secure administrator credentials. The JWT signing key is generated once and persisted in `app.db`.
