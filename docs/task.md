...
- [x] 每次实现提交同步更新任务、测试和必要文档
- [x] 不以跳过测试、放宽隔离或伪造成功结果的方式勾选任务
...
- [x] 实现管理员 JWT、HttpOnly/SameSite cookie 与同源保护
...
- [x] 创建登录页、应用布局、API client、错误边界和紧凑菜单
- [x] 实现概览健康状态与员工、知识、MCP、飞书连接基础统计
- [x] 创建员工列表/详情、启停和待绑定 identity 绑定界面
- [x] 集成 P4 知识树、编辑器、保存冲突和索引状态界面
- [x] 集成 P5 MCP 客户端、工具白名单和策略界面
- [x] 创建 OpenAI-compatible Chat/Embedding 配置与测试页面
- [x] 创建只读飞书状态页并展示待绑定 identity 入口
- [x] 创建按员工、事件、工具、动作、业务单号和结果筛选的脱敏审计页
- [x] 确认 Web 不提供测试聊天、业务审批或任意 MCP 调用入口
- [x] 编写管理员登录、员工绑定、知识编辑、配置脱敏和菜单 Playwright 测试
- [x] 编写未知用户、绑定、余额查询、制度检索、确认提交和状态查询端到端测试
- [x] 编写两名员工并发、跨用户卡片、过期/取消/重复确认和重启恢复端到端测试
- [x] 故障注入验证飞书断线、MCP 故障、Embedding 故障和旧索引退化
- [x] 快照检查 API、日志和审计不泄露密钥、token 或完整敏感参数
- [x] 编写本地启动、飞书应用配置、Mock OA 演示和数据卷说明
- [x] 验收：README 的最小链路可独立复现，全部首版验收红线通过
- [x] **P7 完成**

## 配置动态化验收

- [x] 模型、飞书和运行参数持久化到 `app.db`，并提供脱敏、revision 和审计 API
- [x] 网页端提供模型、飞书和运行参数配置，保存后热应用
- [x] Compose 固定启动参数和 Mock OA 演示连接，不依赖 `.env` 启动
- [x] 未配置模型或飞书时管理端仍可用
